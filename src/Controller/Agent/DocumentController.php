...
class DocumentController extends AbstractController
{
    ...
    public function metaDonnees(#[MapEntity(id: 'id')] Document $document, Request $request): Response
    {
        $document->setMetaDonnees(
            array_merge(
                $request->getPayload()->has('estAttestation') ? ['estAttestation' => $request->getPayload()->getBoolean('estAttestation')] : [],
                $request->getPayload()->has('typeInstitutionSecuritePublique') ? ['typeInstitutionSecuritePublique' => TypeInstitutionSecuritePublique::from($request->getPayload()->getString('typeInstitutionSecuritePublique'))] : []
            ),
