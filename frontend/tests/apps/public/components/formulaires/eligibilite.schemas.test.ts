...
} from "@/apps/public/components/formulaires/eligibilite.schemas.ts";
import { describe, expect, it } from "vitest";

describe("SchemaEtapeDateDecision", () => {
  it("accepte une date renseignée", () => {
    expect(
      SchemaEtapeDateDecision.safeParse({
        dateDecision: new Date("2023-06-15"),
      }).success,
    ).toBe(true);
  });
...
describe("SchemaEtapeActionContentieuse", () => {
  it("accepte 'non'", () => {
    expect(
      SchemaEtapeActionContentieuse.safeParse({ actionContentieuse: "non" })
        .success,
    ).toBe(true);
  });
  ...
  it("accepte 'oui'", () => {
    expect(
      SchemaEtapeActionContentieuse.safeParse({ actionContentieuse: "oui" })
        .success,
    ).toBe(true);
  });
...
describe("SchemaEtapeTypeDecision", () => {
  ...
  it("rejette un tableau vide", () => {
    ...
    expect(result.success).toBe(false);
    expect(result.error?.issues[0].message).toBe(
      "Veuillez sélectionner au moins une décision",
    );
  });
...
describe("SchemaEtapePiecesProc", () => {
  it("accepte un tableau avec au moins une pièce valide", () => {
    expect(
      SchemaEtapePiecesProc.safeParse({ piecesProc: ["acte_introductif"] })
        .success,
    ).toBe(true);
  });
  ...
  it("rejette un tableau vide", () => {
    ...
    expect(result.success).toBe(false);
    expect(result.error?.issues[0].message).toBe(
      "Veuillez sélectionner au moins une pièce de procédure",
    );
  });
  ...
  it("rejette une valeur inconnue dans le tableau", () => {
    const result = SchemaEtapePiecesProc.safeParse({
      piecesProc: ["piece_inconnue"],
    });
    expect(result.success).toBe(false);
  });
});
...
describe("SchemaEtapeDiligences", () => {
  it("accepte true", () => {
    expect(
      SchemaEtapeDiligences.safeParse({
        preuvesDiligences: "sans_justificatif",
      }).success,
    ).toBe(true);
  });
  ...
  it("accepte false", () => {
    expect(
      SchemaEtapeDiligences.safeParse({ preuvesDiligences: "pas_de_demarche" })
        .success,
    ).toBe(true);
  });
