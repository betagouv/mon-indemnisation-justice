...
export const TestEligibiliteForm = ({
  ...
  const setAContacteAssurance = (aContacteAssurance: boolean) =>
    setTest({
      avancement:
        test.rapportAuLogement != "locataire"
          ? "a_contacte_assurance"
          : "a_contacte_bailleur",
      rapportAuLogement: test.rapportAuLogement,
      estVise: test.estVise,
  ...
  const estDecide = useMemo(() => {
    ...
    return !!test.rapportAuLogement && test.rapportAuLogement === "locataire"
      ? test.aContacteBailleur != null
      : test.aContacteAssurance != null;
  }, [test]);

  const refQuestionEstVise = useRef<HTMLDivElement>(null);
  const refQuestionEstHebergeant = useRef<HTMLDivElement>(null);
  const refQuestionRapportAuLogement = useRef<HTMLDivElement>(null);
  const refQuestionAContacteAssurance = useRef<HTMLDivElement>(null);
  const refQuestionAContacteBailleur = useRef<HTMLDivElement>(null);
  ...
  const isMobile = useMemo(
    ...
    [windowInnerWidth, breakpointsValues],
  );
  ...
  return (
    <form name="eligibilite-test-form" method="POST">
      ...
      <CallOut title="Avant de remplir le questionnaire">
        ...
        et vérifier votre éligibilité à l'indemnisation, vous devez répondre aux
        questions suivantes.
      </CallOut>

      {estQuestionVisible("rapport_au_logement") && (
        <div className="fr-col-12" ref={refQuestionRapportAuLogement}>
          <RadioButtons
      ...
      {estQuestionVisible("est_vise") && (
        <div className="fr-col-12">
          <RadioButtons
            legend={
              <>
                {test.rapportAuLogement == "bailleur" ? (
                  <>
                    Est-ce que votre locataire était la personne recherchée par
                  </>
                ) : (
                  <>Étiez-vous la personne recherchée par </>
                )}{" "}
                <Tooltip
      ...
      {estQuestionVisible("est_hebergeant") && (
        <div className="fr-col-12" ref={refQuestionEstHebergeant}>
          <RadioButtons
            legend={
              test.rapportAuLogement == "bailleur"
                ? "Est-ce que la personne recherchée par les forces de l'ordre est hébergée par votre locataire ou est son sous-locataire ?"
      ...
      {estQuestionVisible("a_contacte_assurance") && (
        <div className="fr-col-12" ref={refQuestionAContacteAssurance}>
          <RadioButtons
      ...
      {estQuestionVisible("a_contacte_bailleur") &&
        test.rapportAuLogement === "locataire" && (
          <div className="fr-col-12" ref={refQuestionAContacteBailleur}>
            <RadioButtons
            ...
            <Alert
              ...
              description={
                <p>
                  ...
                  &nbsp;au cours de leur intervention. Il vous appartiendra de
                  prouver cette faute.
                </p>
              }
            />
