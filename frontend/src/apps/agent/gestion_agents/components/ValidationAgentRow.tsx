...
export const ValidationAgentRow = observer(
  ({
    validation,
    editable = false,
  }: {
      ...
      <tr key={`row-${validation.agent.id}`}>
        <td className="fr-col-3 fr-grid-row fr-grid-row--gutters">
          <p>
            ...
            <br />
            <span className="fr-text--sm">{validation.agent.courriel}</span>
          </p>
        </td>
        <td className="fr-col-4 fr-grid-row fr-grid-row--gutters">
          {validation.administration ? (
            <p>
              Rattaché{" "}
              {validation.administration.estLibelleFeminin ? "à la" : "au"}
              ...
              {!validation.agent.administration && (
                <a
                  ...
                  role="button"
                  title="Changer"
                  onClick={() => validation.reinitialiser()}
                ></a>
              )}
              <br />
              {validation.agent.datePremiereConnexion && (
                <span className="fr-text--sm">
                  ...
                  {validation.agent.datePremiereConnexion.toLocaleString(
                    "fr-FR",
                    {
                      weekday: "long",
                      day: "numeric",
            ...
            <div className="fr-select-group fr-select-group--error">
              <label
                className="fr-label"
                htmlFor={`selection-administration-${validation.agent.id}`}
              >
                Rattachement :
              </label>
              <select
                ...
                onChange={(e) =>
                  (validation.administration = Administration.resoudre(
                    e.target.value,
                  ))
                }
              >
                ...
                {Administration.catalog
                  ...
                  .toArray()
                  .map((administration) => (
                    <option value={administration.id} key={administration.id}>
          ...
          {validation.administration && (
            ...
            >
              ...
              {validation.administration?.roles.map((role: string) => (
                <div
                  key={`permission-${validation.agent.id}-${role.toLowerCase()}`}
                ...
                >
                  <div className="fr-checkbox-group">
                    <input
                      ...
                      onChange={(e) =>
                        validation.definirRole(role, e.target.checked)
                      }
