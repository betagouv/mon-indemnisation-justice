...
export function StepActionContentieuse({ reponses, onPrecedent, onSuivant, isLastStep }: StepProps) {
  const formulaire = useForm({
    validators: { onSubmit: SchemaEtapeActionContentieuse },
    defaultValues: { actionContentieuse: reponses.actionContentieuse } as { actionContentieuse?: ActionContentieuse },
    onSubmit: async ({ value, formApi }) => {
      if (formApi.state.isValid) {
        onSuivant({ actionContentieuse: value.actionContentieuse! });
      }
    },
  });
