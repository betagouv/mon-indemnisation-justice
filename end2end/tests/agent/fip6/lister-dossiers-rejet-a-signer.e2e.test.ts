...
test("lister dossiers dont le courrier de rejet est à signer", async ({
  ...
  try {
    ...
    await page.waitForURL("/agent/fip6/dossiers");

    await expect(getTitre(page, "Les dossiers")).toBeVisible();

    await page.goto("/agent/fip6/dossiers/rejet-a-signer");

    await expect(
      getTitre(page, "Dossiers en attente de signature du courrier de rejet"),
    ).toBeVisible();
