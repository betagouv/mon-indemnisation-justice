...
createApp({
  ...
  questions: {
    description: function (reponses, decisions) {
      return reponses.departement != null;
    },
    estVise: function (reponses, decisions) {
      return reponses.description != null;
    },
    estHebergeant: function (reponses, decisions) {
      return (
        reponses.estVise != null &&
        (decisions.length === 0 || reponses.estHebergeant != null)
      );
    },
    estProprietaire: function (reponses, decisions) {
      return (
        reponses.estHebergeant != null &&
        (decisions.length === 0 || reponses.estProprietaire != null)
      );
    },
    aContacteAssurance: function (reponses, decisions) {
      return (
        reponses.estProprietaire != null &&
        (decisions.length === 0 || reponses.aContacteAssurance != null)
      );
    },
    aContacteBailleur: function (reponses, decisions) {
      return (
        reponses.aContacteAssurance != null &&
        reponses.estProprietaire === false &&
        (decisions.length === 0 || reponses.aContacteBailleur != null)
      );
    },
  },
  decisions: [],
  estDecide: function (nom) {
    return !!this.decisions.find(function (decision) {
      return decision.name === nom;
    });
  },
  estEligible() {
    ...
    if (reponses.estVise) {
      ...
      if (reponses.estProprietaire) {
        if (reponses.aContacteAssurance != null) {
          this.decisions.push({
            name: reponses.aContacteAssurance
              ? "success"
              : "warning_contact_assurance",
          });
        }
      } else if (reponses.estProprietaire === false) {
        if (
          reponses.aContacteAssurance != null &&
          reponses.aContacteBailleur != null
        ) {
  ...
  reinitialiser: function (question) {
    if (question) {
      ...
      const questionsSuivantes = questions.slice(
      ...
      setTimeout(() => {
        ...
        if (questions.at(questions.indexOf(question) + 1)) {
          element = document.getElementById(
            `eligibilite-test-form-question-${questions.at(questions.indexOf(question) + 1)}`,
          );
        } else {
