import {Document, DocumentType, Redacteur} from "@/apps/agent/dossiers/models";
import {DossierDetail} from "@/apps/agent/dossiers/models/Dossier";
import {observer} from "mobx-react-lite";
import React, {useState} from 'react';

export const ConsultationDossierApp = observer(({dossier}: { dossier: DossierDetail }) => {

    const [attributaire, attribuer] = useState(dossier.redacteur)
    const [modeAttribution, setModeAttribution] = useState(false)
    const [sauvegarderEnCours, setSauvegarderEnCours] = useState(false)

    const validerAttribution = async () => {
        setSauvegarderEnCours(true);
        const response = await fetch(`/agent/redacteur/dossier/${dossier.id}/attribuer.json`, {
            method: 'POST',
            headers: {
                ...
                <div className="fr-grid-row fr-grid-row--gutters">
                    ...
                    <div className="fr-col-12 fr-p-3w">
                        ...
                        <div className="fr-dossier-etat fr-p-4w">
                            ...
                            <div className="">
                                <p className="fr-badge fr-badge--info fr-badge--no-icon fr-py-1w fr-px-2w">À statuer</p>
                            </div>
                            ...
                            <div>
                                {modeAttribution ?
                                    <>
                                        ...
                                        <div className="fr-select-group fr-col-offset-6 fr-col-lg-6 fr-mb-0">
                                            ...
                                            <select
                                                ...
                                                onChange={(e) => attribuer(Redacteur.resoudre(parseInt(e.target.value)))}
                                            >
                                                <option value="" disabled hidden>Sélectionnez un rédacteur</option>
                                                {Redacteur.catalog.values().toArray().map((redacteur: Redacteur) =>
                                                    <option value={redacteur.id}
                                                            key={redacteur.id}>{redacteur.nom}</option>
                                                )}
                                            </select>
                                        </div>
                                        ...
                                        <ul className="fr-btns-group fr-btns-group--sm fr-btns-group--inline fr-btns-group--right fr-mt-3w">
                                            ...
                                            <li>
                                                <button
                                                    ...
                                                    type="button"
                                                    disabled={sauvegarderEnCours || !attributaire || attributaire === dossier.redacteur?.id}
                                    ...
                                    <p className="fr-m-1w">Ce dossier
                                        {dossier.redacteur ?
                                            <> est attribué à <u> {dossier.redacteur.nom} </u></> :
                                            <> n'est <i>pas encore attribué</i> à un rédacteur </>
                                        } <a role="button" className="fr-link" onClick={() => setModeAttribution(true)}>
