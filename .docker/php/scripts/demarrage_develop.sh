...
if [ -z "${INSTANCE_NUMBER}" ] || [ ${INSTANCE_NUMBER} == '0' ]; then
  ...
  "${ROOT}/bin/console" doctrine:fixtures:load --no-interaction --purge-exclusions=geo_codes_postaux,geo_communes,geo_departements,geo_pays,geo_regions,agent_fournisseurs_identites
fi
