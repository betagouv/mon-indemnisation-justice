...
if [ -z "${INSTANCE_NUMBER}" ] || [ ${INSTANCE_NUMBER} == '0' ]; then
  ...
  "${ROOT}/bin/console" doctrine:fixtures:load --no-interaction --purge-exclusions=geo_codes_postaux,geo_communes,geo_departements,geo_pays,geo_regions,"${ROOT}/bin/console" doctrine:fixtures:load --no-interaction --purge-exclusions=geo_codes_postaux,geo_communes,geo_departements,geo_pays,geo_regions,fdo_etablissements,fdo_etablissements_code_postaux
fi
